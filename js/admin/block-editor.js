jQuery( document ).ready( function( $ ) {
	...
	$( 'input[name="_develope-full-width-content"]' ).on( 'change', function() {
		if ( 'true' === this.value ) {
	...
	$( 'input[name="_develope-sidebar-layout-meta"]' ).on( 'change', function() {
		if ( 'true' !== $( 'input[name="_develope-full-width-content"]:checked' ).val() ) {
			var this_value = this.value,
				content_width = '',
				calc = '',
				container_width = develope_block_editor.container_width,
				right_sidebar_width = develope_block_editor.right_sidebar_width,
				left_sidebar_width = develope_block_editor.left_sidebar_width,
				right_content_padding = develope_block_editor.content_padding_right,
				left_content_padding = develope_block_editor.content_padding_left;

			if ( '' === this_value ) {
				this_value = develope_block_editor.global_sidebar_layout;
			}
	...
	var disable_content_title_input = $( '#meta-develope-disable-headline' );
	var disable_content_title_button = $( 'button.content-title-visibility' );
	var body = $( 'body' );

	if ( 'false' === develope_block_editor.content_title ) {
	...
	if ( develope_block_editor.show_editor_styles ) {
		var text_color = tinycolor( develope_block_editor.text_color ).toHex8(),
...
jQuery( window ).load( function() {
	var post_title_block = jQuery( '.editor-post-title__block' ),
		disable_title_option = jQuery( '#meta-develope-disable-headline' );

	if ( post_title_block && disable_title_option.length > 0 ) {
		post_title_block.append( '<button class="content-title-visibility disable-content-title" title="' + develope_block_editor.disable_content_title + '" aria-hidden="true"></button>' );
		post_title_block.append( '<button class="content-title-visibility show-content-title" title="' + develope_block_editor.show_content_title + '" aria-hidden="true"></button>' );
	}
} );
