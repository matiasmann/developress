...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Develope_Customize_Width_Slider_Control' ) ) {
	/**
	 * Create our container width slider control
	 * @deprecated 1.3.47
	 */
	class Develope_Customize_Width_Slider_Control extends WP_Customize_Control {
		public function render_content() {}
	}
}

if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'DevelopeLabelControl' ) ) {
	/**
	 * Heading area
	 * @since 0.1
	 * @depreceted 1.3.41
	 **/
	class DevelopeLabelControl extends WP_Customize_Control {
		public function render_content() {}
	}
}

if ( ! class_exists( 'Develope_Google_Font_Dropdown_Custom_Control' ) ) {
	/**
	 * A class to create a dropdown for all google fonts
	 */
	class Develope_Google_Font_Dropdown_Custom_Control extends WP_Customize_Control {
		public $type = 'dp-customizer-fonts';

		public function enqueue() {
			wp_enqueue_script( 'developress-customizer-fonts', trailingslashit( get_template_directory_uri() ) . 'inc/js/typography-controls.js', array( 'customize-controls' ), DEVELOPE_VERSION, true );
			wp_localize_script( 'developress-customizer-fonts', 'dp_customize', array( 'nonce' => wp_create_nonce( 'dp_customize_nonce' ) ) );
		}
		...
		public function to_json() {
			...
			$this->json['default_fonts_title'] = __( 'Default fonts', 'developress' );
...
if ( ! class_exists( 'Develope_Select_Control' ) ) {
	/**
	 * A class to create a dropdown for font weight
	 */
	class Develope_Select_Control extends WP_Customize_Control {
		public $type = 'dp-typography-select';
...
if ( ! class_exists( 'Develope_Hidden_Input_Control' ) ) {
	/**
	 *	Create our hidden input control
	 */
	class Develope_Hidden_Input_Control extends WP_Customize_Control {
		// Setup control type
		public $type = 'dp-hidden-input';
		public $id = '';
		...
		public function content_template() {
			?>
			<input name="{{ data.id }}" type="text" {{{ data.link }}} value="{{{ data.value }}}" class="dp-hidden-input" />
			<?php
		}
	}
}

if ( ! class_exists( 'Develope_Font_Weight_Custom_Control' ) ) {
	/**
	 * A class to create a dropdown for font weight
	 * @deprecated since 1.3.40
	 */
	class Develope_Font_Weight_Custom_Control extends WP_Customize_Control {
...
if ( ! class_exists( 'Develope_Text_Transform_Custom_Control' ) ) {
	/**
	 * A class to create a dropdown for text-transform
	 * @deprecated since 1.3.40
	 */
	class Develope_Text_Transform_Custom_Control extends WP_Customize_Control {
...
if ( ! class_exists( 'Develope_Customize_Slider_Control' ) ) {
	/**
	 * Create our container width slider control
	 * @deprecated 1.3.47
	 */
	class Develope_Customize_Slider_Control extends WP_Customize_Control {
		public function render_content() {}
	}
}
