( function( api ) {
	...
	api( 'develope_settings[nav_position_setting]', function( setting ) {
		...
		api.control( 'develope_settings[nav_drop_point]', linkSettingValueToControlActiveState );
	} );

	var setOption = function( headerAlignment, navLocation, navAlignment ) {
		if ( headerAlignment ) {
			api.control( 'develope_settings[header_alignment_setting]' ).setting.set( headerAlignment );
		}

		if ( navLocation ) {
			api.control( 'develope_settings[nav_position_setting]' ).setting.set( navLocation );
		}

		if ( navAlignment ) {
			api.control( 'develope_settings[nav_alignment_setting]' ).setting.set( navAlignment );
		}
	};

	api( 'develope_header_helper', function( value ) {
		var headerAlignment = false,
			navLocation = false,
			navAlignment = false;

		value.bind( function( newval ) {
			var headerAlignmentSetting = api.control( 'develope_settings[header_alignment_setting]' ).setting;
			var navLocationSetting = api.control( 'develope_settings[nav_position_setting]' ).setting;
			var navAlignmentSetting = api.control( 'develope_settings[nav_alignment_setting]' ).setting;
			...
			if ( 'default' === newval ) {
				setOption( developress_defaults.header_alignment_setting, developress_defaults.nav_position_setting, developress_defaults.nav_alignment_setting );
			}
	...
	api( 'nav_color_presets', function( value ) {
		...
		value.bind( function( newval ) {
			var backgroundColorSetting = api.instance( 'develope_settings[navigation_background_color]' ),
				textColorSetting = api.instance( 'develope_settings[navigation_text_color]' ),
				backgroundColorHoverSetting = api.instance( 'develope_settings[navigation_background_hover_color]' ),
				textColorHoverSetting = api.instance( 'develope_settings[navigation_text_hover_color]' ),
				currentBackgroundColorSetting = api.instance( 'develope_settings[navigation_background_current_color]' ),
				currentTextColorSetting = api.instance( 'develope_settings[navigation_text_current_color]' ),
				subMenuBackgroundColorSetting = api.instance( 'develope_settings[subnavigation_background_color]' ),
				subMenuTextColorSetting = api.instance( 'develope_settings[subnavigation_text_color]' ),
				subMenuBackgroundColorHoverSetting = api.instance( 'develope_settings[subnavigation_background_hover_color]' ),
				subMenuTextColorHoverSetting = api.instance( 'develope_settings[subnavigation_text_hover_color]' ),
				subMenuCurrentBackgroundColorSetting = api.instance( 'develope_settings[subnavigation_background_current_color]' ),
				subMenuCurrentTextColorSetting = api.instance( 'develope_settings[subnavigation_text_current_color]' );
			...
			if ( 'default' === newval ) {
				backgroundColorSetting.set( developress_color_defaults.navigation_background_color );
				textColorSetting.set( developress_color_defaults.navigation_text_color );

				backgroundColorHoverSetting.set( developress_color_defaults.navigation_background_hover_color );
				textColorHoverSetting.set( developress_color_defaults.navigation_text_hover_color );

				currentBackgroundColorSetting.set( developress_color_defaults.navigation_background_current_color );
				currentTextColorSetting.set( developress_color_defaults.navigation_text_current_color );

				subMenuBackgroundColorSetting.set( developress_color_defaults.subnavigation_background_color );
				subMenuTextColorSetting.set( developress_color_defaults.subnavigation_text_color );

				subMenuBackgroundColorHoverSetting.set( developress_color_defaults.subnavigation_background_hover_color );
				subMenuTextColorHoverSetting.set( developress_color_defaults.subnavigation_text_hover_color );

				subMenuCurrentBackgroundColorSetting.set( developress_color_defaults.subnavigation_background_current_color );
				subMenuCurrentTextColorSetting.set( developress_color_defaults.subnavigation_text_current_color );
			}
