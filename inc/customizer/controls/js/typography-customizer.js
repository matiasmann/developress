( function( api ) {
	...
	api.controlConstructor['dp-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.developress-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the develope_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'develope_get_all_google_fonts_ajax',
								dp_customize_nonce: dp_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.developress-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.developress-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.developress-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.developress-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.developress-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.developress-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.developress-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.developress-font-variant' );
			...
			control.container.on( 'change', '.developress-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.developress-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.developress-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.developress-font-family select' ).select2();

	$( '.developress-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".developress-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.developress-font-variant' ).hide();
		}
	} );
