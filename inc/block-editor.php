<?php
/**
 * Integrate DeveloPress with the WordPress block editor.
 *
 * @package DeveloPress
 */
 ...
 * We need this function as the post meta in develope_get_layout() only runs
 * on is_singular()
 *
 * @since 2.2
 *
 ...
 */
function develope_get_block_editor_sidebar_layout( $meta = true ) {
	$layout = develope_get_option( 'layout_setting' );

	if ( function_exists( 'get_current_screen' ) ) {
		$screen = get_current_screen();

		if ( is_object( $screen ) && 'post' === $screen->post_type ) {
			$layout = develope_get_option( 'single_layout_setting' );
		}
	}

	// Add in our default filter in case people have adjusted it.
	$layout = apply_filters( 'develope_sidebar_layout', $layout );

	if ( $meta ) {
		$layout_meta = get_post_meta( get_the_ID(), '_develope-sidebar-layout-meta', true );
	...
	return apply_filters( 'develope_block_editor_sidebar_layout', $layout );
}

/**
 * Check whether we're disabling the content title or not.
 * We need this function as the post meta in develope_show_title() only runs
 * on is_singular()
 *
 * @since 2.2
 */
function develope_get_block_editor_show_content_title() {
	$title = develope_show_title();
	...
	return apply_filters( 'develope_block_editor_show_content_title', $title );
}
 ...
 * @since 2.2
 */
function develope_get_block_editor_content_width() {
	...
	$layout = develope_get_block_editor_sidebar_layout();
	...
	return apply_filters( 'develope_block_editor_content_width', $content_width );
}

add_action( 'enqueue_block_editor_assets', 'develope_enqueue_google_fonts' );
add_action( 'enqueue_block_editor_assets', 'develope_enqueue_backend_block_editor_assets' );
/**
 * Add CSS to the admin side of the block editor.
 *
 * @since 2.2
 */
function develope_enqueue_backend_block_editor_assets() {
	wp_enqueue_style( 'develope-block-editor-styles', get_template_directory_uri() . "/css/admin/block-editor.css", false, DEVELOPE_VERSION, 'all' );
	wp_enqueue_script( 'develope-block-editor-tinycolor', get_template_directory_uri() . "/js/admin/tinycolor.js", false, DEVELOPE_VERSION, true );
	wp_enqueue_script( 'develope-block-editor-scripts', get_template_directory_uri() . "/js/admin/block-editor.js", array( 'jquery', 'develope-block-editor-tinycolor' ), DEVELOPE_VERSION, true );

	$show_editor_styles = apply_filters( 'develope_show_block_editor_styles', true );

	if ( $show_editor_styles ) {
		wp_add_inline_style( 'develope-block-editor-styles', develope_do_inline_block_editor_css() );
	}
	...
	wp_localize_script( 'develope-block-editor-scripts', 'develope_block_editor', array(
		'global_sidebar_layout' => develope_get_block_editor_sidebar_layout( false ),
		'container_width' => develope_get_option( 'container_width' ),
		'right_sidebar_width' => apply_filters( 'develope_right_sidebar_width', '25' ),
		'left_sidebar_width' => apply_filters( 'develope_left_sidebar_width', '25' ),
		...
		'content_title' => develope_get_block_editor_show_content_title() ? 'true' : 'false',
		'disable_content_title' => esc_html( 'Disable Content Title', 'developress' ),
		'show_content_title' => esc_html( 'Show Content Title', 'developress' ),
 ...
 * @since 2.2
 */
function develope_do_inline_block_editor_css() {
	$color_settings = wp_parse_args(
		get_option( 'develope_settings', array() ),
		develope_get_color_defaults()
	);
	...
	$content_width = develope_get_block_editor_content_width();
	...
	if ( 'true' === get_post_meta( get_the_ID(), '_develope-full-width-content', true ) ) {
	...
	if ( apply_filters( 'develope_do_group_inner_container_style', true ) ) {
		...
		$css->add_property( 'padding', develope_padding_css( $spacing_settings['content_top'], $spacing_settings['content_right'], $spacing_settings['content_bottom'], $spacing_settings['content_left'] ) );
	}
	...
	if ( $color_settings['content_text_color'] ) {
		...
		$css->add_property( 'color', esc_attr( develope_get_option( 'text_color' ) ) );
	}
	...
	$css->add_property( 'background-color', esc_attr( develope_get_option( 'background_color' ) ) );

	if ( $color_settings['content_background_color'] ) {
		$body_background = esc_attr( develope_get_option( 'background_color' ) );
	...
	if ( $color_settings['content_link_color'] ) {
		...
		$css->add_property( 'color', esc_attr( develope_get_option( 'link_color' ) ) );
	}
	...
	if ( $color_settings['content_link_hover_color'] ) {
		...
		$css->add_property( 'color', esc_attr( develope_get_option( 'link_color_hover' ) ) );
	}
