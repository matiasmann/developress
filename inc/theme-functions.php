...
if ( ! function_exists( 'develope_get_layout' ) ) {
	/**
	...
	function develope_get_layout() {
		...
		if ( is_home() || is_archive() || is_search() || is_tax() ) {
			$layout = develope_get_option( 'blog_layout_setting' );
		}

		return apply_filters( 'develope_sidebar_layout', $layout );
	}
}

if ( ! function_exists( 'develope_get_footer_widgets' ) ) {
	/**
	 ...
	 */
	function develope_get_footer_widgets() {
		$widgets = develope_get_option( 'footer_widget_setting' );

		if ( is_singular() ) {
			$widgets_meta = get_post_meta( get_the_ID(), '_develope-footer-widget-meta', true );
		...
		return apply_filters( 'develope_footer_widgets', $widgets );
	}
}

if ( ! function_exists( 'develope_show_excerpt' ) ) {
	/**
	 * Figure out if we should show the blog excerpts or full posts
	 * @since 1.3.15
	 */
	function develope_show_excerpt() {
		global $post;

		// Check to see if the more tag is being used.
		$more_tag = apply_filters( 'develope_more_tag', strpos( $post->post_content, '<!--more-->' ) );

		$format = ( false !== get_post_format() ) ? get_post_format() : 'standard';

		$show_excerpt = ( 'excerpt' === develope_get_option( 'post_content' ) ) ? true : false;
		...
		return apply_filters( 'develope_show_excerpt', $show_excerpt );
	}
}

if ( ! function_exists( 'develope_show_title' ) ) {
	/**
	 ...
	 */
	function develope_show_title() {
		return apply_filters( 'develope_show_title', true );
	}
}

if ( ! function_exists( 'develope_get_premium_url' ) ) {
	/**
	 ...
	 * @return string The URL to developress.com.
	 */
	function develope_get_premium_url( $url = 'https://developress.org/premium', $trailing_slash = true ) {
		if ( $trailing_slash ) {
			$url = trailingslashit( $url );
		}

		$args = apply_filters( 'develope_premium_url_args', array(
			'ref' => null,
			'campaign' => null,
		) );
...
if ( ! function_exists( 'develope_padding_css' ) ) {
	/**
	 ...
	 */
	function develope_padding_css( $top, $right, $bottom, $left ) {
...
if ( ! function_exists( 'develope_get_link_url' ) ) {
	/**
	 * Return the post URL.
	 *
...
if ( ! function_exists( 'develope_get_navigation_location' ) ) {
	/**
	 ...
	 */
	function develope_get_navigation_location() {
		return apply_filters( 'develope_navigation_location', develope_get_option( 'nav_position_setting' ) );
	}
}
 ...
 * @since 2.3
 */
function develope_has_logo_site_branding() {
	if ( get_theme_mod( 'custom_logo' ) && ( ! develope_get_option( 'hide_title' ) || ! develope_get_option( 'hide_tagline' ) ) ) {
		return true;
	}
 ...
 * @since 2.3
 */
function develope_get_svg_icon( $icon, $replace = false ) {
	...
	$output = apply_filters( 'develope_svg_icon_element', $output, $icon );
	...
	return apply_filters( 'develope_svg_icon', $output, $icon );
}
 ...
 * @return string
 */
function develope_do_svg_icon( $icon, $replace = false ) {
	echo develope_get_svg_icon( $icon, $replace );
}
 ...
 */
function develope_get_media_query( $name ) {
	$desktop = apply_filters( 'develope_desktop_media_query', '(min-width:1025px)' );
	$tablet = apply_filters( 'develope_tablet_media_query', '(min-width: 769px) and (max-width: 1024px)' );
	$mobile = apply_filters( 'develope_mobile_media_query', '(max-width:768px)' );
	$mobile_menu = apply_filters( 'develope_mobile_menu_media_query', $mobile );

	$queries = apply_filters( 'develope_media_queries', array(
		'desktop' 		=> $desktop,
		'tablet' 		=> $tablet,
		'mobile' 		=> $mobile,
